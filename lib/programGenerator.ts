...
function repsForExercise(ex: ExerciseDef, profile: UserProfile, defaultReps: number): number {
  ...
  // Cardio-pattern exercises are handled entirely by cardioPrescriptionFor()
  // below — the distance/duration already lives in the exercise name (e.g.
  // "3 Mile Run", "20-Minute Cardio"), so reps/sets here don't apply to them.
  return defaultReps;
}
