...
export default function LogPage() {
  ...
  const [coachBriefing, setCoachBriefing] = useState("");
  ...
  useEffect(() => {
    if (!coachVoiceReady || !profile || !program || !nextSession) return;
    void speakCoachIntro();
  }, [coachVoiceReady, profile, program, nextSession]);
  ...
  async function speakCoachIntro() {
    if (!coachVoiceReady || typeof window === "undefined" || !profile) return;
    const focusText = nextSession?.focus || "your workout";
    const fallbackMessage = `Coach here. Today’s focus is ${focusText}. Start with the first exercise, keep your form tight, and work with intention.`;
    const sessionExercises = nextSession
      ? [{ focus: nextSession.focus, exercises: nextSession.exercises.map((exercise) => ({ exerciseId: exercise.exerciseId, sets: exercise.sets, reps: exercise.reps })) }]
      : undefined;

    let message = fallbackMessage;
    try {
      const data = await postJson<{ briefing?: string }>('/api/coach-briefing', {
        profile,
        sessionFocus: focusText,
        sessionExercises,
      });
      if (data.briefing) {
      ...
      console.error("Coach briefing failed", err);
    }

    setCoachBriefing(message);
  ...
  return (
    <AppShell title="Log session">
      <main className="px-4 sm:px-6 lg:px-10 py-6 max-w-2xl mx-auto w-full">
        ...
        {coachBriefing ? (
          <p className="text-sm text-signal mb-6 rounded-2xl border border-signal/20 bg-signal/10 px-3 py-3">{coachBriefing}</p>
        ) : (
          <div className="mb-6" />
        )}
