...
import { generateProgramAndCoachBriefing } from "@/lib/groq";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // 3. Run the (potentially expensive) LLM calls in parallel.
    const { program, coachBriefing, planSummary } = await generateProgramAndCoachBriefing(profile);
    return NextResponse.json({ program, coachBriefing, planSummary });
  } catch (err) {
