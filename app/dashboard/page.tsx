...
export default function DashboardPage() {
  ...
  const coachSummary = profile.aiPlanSummary?.trim() || profile.specificGoals?.trim() || `Your plan will focus on ${profile.goals.join(", ")} with ${profile.daysPerWeek} training day${profile.daysPerWeek === 1 ? "" : "s"} per week.`;
    ...
    <AppShell>
      <main className="px-4 sm:px-6 lg:px-10 py-6 lg:py-8 max-w-6xl mx-auto w-full">
        ...
        <section className="mb-6 rounded-3xl border border-border/70 bg-surface/90 p-5 shadow-soft">
          <p className="text-sm font-semibold text-ink">Coach summary</p>
          <p className="mt-2 text-sm leading-relaxed text-ink">{coachSummary}</p>
        </section>
